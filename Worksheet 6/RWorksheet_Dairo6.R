...
cyl_Vs_Displ <- ggplot(mpg, aes(x = cyl, y = displ, color = displ)) +
  geom_point()
print(Cyl_Vs_Displ + ggtitle("Relationship between No. of Cylinders and Engine Displacement"))
...
ggplot(data = mpg) +
geom_point(mapping = aes(x = displ, y = hwy), colour = "blue")
# - Putting the values inside the aes generates a legend and makes the color with that while 
# putting it outside aes ggplot2 did not make the legend automatically and inputted your value.
